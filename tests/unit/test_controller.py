...
async def test_get_media_player_state_exception(controller, mock_client):
    ...
    # Arrange
    mock_client.get.side_effect = httpx.ConnectTimeout("Network Connection Timeout")
...
async def test_get_active_app_success(controller, mock_client):
    ...
    mock_response = MagicMock()
    mock_response.status_code = HTTPStatus.OK
    mock_response.text = expected_app
    mock_client.get.return_value = mock_response

    # Act
    active_app_result = await controller.get_active_app()

    # Assert
    assert active_app_result == expected_app
